...
import { getTwitterFolloweUserName, getTwitterShareUserInfo, getTwitterShareUserInfoCount } from 'apis'
import {
...
const Search: React.FC<Props> = ({ onSearch, setRate,setList, setSearchLoaded, tabIndex: index }) => {
  ...
  const [searchTypeEl, setSearchTypeEl] = React.useState<null | HTMLElement>(null)
  ...
  const handleSearch = async () => {
    ...
    if (tabIndex === 4 && index === 3 && selectType === SearchType.Account) {
      const contractRes = await searchGithubAccount(CONNECTIDS.user, identity, searchVal)
      onSearch(contractRes)
      setSearchLoaded(true)
      setLoading(false)
    } else if (tabIndex === 3 && index === 3 && selectType === SearchType.Account) {

      const contractRes = await searchDiscordAccount(CONNECTIDS.user, identity, searchVal)
      onSearch(contractRes)
      setSearchLoaded(true)
      setLoading(false)
    } else if (tabIndex === 1 && index === 3 && selectType === SearchType.Account) {

      const contractRes = await searchIcpAccount(CONNECTIDS.user, identity, searchVal)
      onSearch(contractRes)
      setSearchLoaded(true)
      setLoading(false)
    } else if (index === 3 && selectType === SearchType.Followers && tabIndex === 2) {

      getTwitterFolloweUserName(searchVal).then(async res => {
        if (res.code === 200) {
          const names = res.data || []
          setLoading(false)
          const contractRes = await searchMultiTwitterAccount(CONNECTIDS.user, identity, names)
    ...
    } else if (index === 3 && selectType === SearchType.TweetUrl && tabIndex === 2) {
    ...
    } else if (index === 3 && selectType === SearchType.Account && tabIndex === 2) {
      const contractRes = await searchTwitterAccount(CONNECTIDS.user, identity, searchVal)
      onSearch(contractRes)
      setSearchLoaded(true)
      setLoading(false)
    } else if (index === 3 && selectPlatform === Platform.Twitter && tabIndex === 2) {
      const contractRes = await searchTwitterAccount(CONNECTIDS.user, identity, searchVal)
      onSearch(contractRes)
      setSearchLoaded(true)
      setLoading(false)
    ...
    <div className='search'>
...

      {
        ...
        <div className='searchtab'>
          <div
            className={cls('searchtab_item', { active: tabIndex === 1, disabled: false })}
            onClick={() => setTabIndex(1)}>ICP Address
          </div>
          <div
            className={cls('searchtab_item', { active: tabIndex === 2 })}
            onClick={() => setTabIndex(2)}>Twitter
          </div>
          <div
            className={cls('searchtab_item', { active: tabIndex === 3, disabled: false })}
            onClick={() => setTabIndex(3)}>Discord
          </div>
          <div
            className={cls('searchtab_item', { active: tabIndex === 4, disabled: false })}
            onClick={() => setTabIndex(4)}>Github
          </div>
        </div>
      }
      ...
      <div className='search_bottom'>
        ...
        <Menu open={openType} anchorEl={searchTypeEl} onClose={handleTypeClose} className='menu'>
          <MenuItem
        ...
        <Menu open={open} anchorEl={anchorEl} onClose={handleClose} className='menu'>
          <MenuItem
            className='seletItem'
        ...
        <div className='search_inputWrap'>
          ...
          <Button
            ...
            onClick={handleSearch}>{loading ?
            <ThreeDots width={44} /> : 'Search'} </Button>
        </div>
      </div>
    </div>
  )
