...
import {
  getVerifyGithubUserInfo,
  getVerifyDiscordUserInfo,
  getVerifyTwitterUserInfo,
} from "apis";
...
const Legalize: React.FC<Props> = () => {
  ...
  const verifyGithub = async (code: string) => {
    getVerifyGithubUserInfo(code).then(async (res) => {
      if (res.code === 200) {
        const name = res.data;
        const contractRes = await verify(identity, { github: null }, name);
        if (contractRes) {
  ...
  const verifyDiscord = async (code: string) => {
    getVerifyDiscordUserInfo(code)
      .then(async (res) => {
        if (res.code === 200) {
          const name = res.data;
          const contractRes = await verify(identity, { discord: null }, name);
          if (contractRes) {
  ...
  const verifyTwitter = async (code: string) => {
    getVerifyTwitterUserInfo(code)
      .then(async (res) => {
        if (res.code === 200) {
          const name = res.data;
          const contractRes = await verify(identity, { twitter: null }, name);
          if (contractRes) {
  ...
  return (
    <div className="legalize">
      {loading ? (
        ...
        <a
          className={cls("legalize-item twitter", { verify: twitterVerify })}
          href={certifiedTwitterUrl(WEBSITE_URL)}
        >
          {twitterVerify && <div className="verifyStatus">Verified</div>}
          <TwitterIcon />
          <div className="legalize-item__text">
            {twitterVerify ? twitterVerify : "Twitter"}
          </div>
        </a>
      )}
      {loading ? (
        ...
        >
          {discordVerify && <div className="verifyStatus">Verified</div>}
          <DiscordIcon />
          <div className="legalize-item__text">
            {discordVerify ? discordVerify : "Discord"}
          </div>
        </a>
      )}
      {loading ? (
        ...
        <a
          className={cls("legalize-item github", { verify: githubVerify })}
          href={certifiedGithubUrl(WEBSITE_URL)}
        >
          {githubVerify && <div className="verifyStatus">Verified</div>}
          <GithubIcon />
          <div className="legalize-item__text">
            {githubVerify ? githubVerify : "Github"}
          </div>
        </a>
      )}
    </div>
