...
export class Transaction {
  ...
  constructor(rosettaTransaction, blockIndex) {
    ...
    if (operations.length >= 1) {
      ...
      if ((operations[0].type === "TRANSACTION" || operations[0].type === "BURN") && !this.amount.isZero()) {
    ...
    if (operations.length >= 3 && operations[2].type === "FEE") this.fee = new BigNumber(-operations[2].amount.value);
...
export default class RosettaApi {
  ...
  constructor() {
    ...
    this.networkIdentifier = this.networksList().then((res) =>
      res.network_identifiers.find((networkIdentifier) => networkIdentifier.blockchain === "Internet Computer")
    );
  }
  ...
  async getAccountBalance(accountAddress) {
    try {
      ...
      //console.log(error);
      return new RosettaError(error.message, axios.isAxiosError(error) ? error?.response?.status : undefined);
    }
  }
  ...
  async getTransaction(transactionHash) {
    try {
      ...
      if (responseTransactions.transactions.length === 0) return new RosettaError("Transaction not found.", 500);
  ...
  async getTransactions(limit, maxBlockIndex, offset) {
    try {
      ...
      const transactions = [];
      for (let i = 0; i < transactionCount; i++) transactions.push(await this.getTransactionByBlock(blockIndex - i));
      return transactions;
    } catch (error) {
      //console.log(error);
      return new RosettaError(error.message, axios.isAxiosError(error) ? error?.response?.status : undefined);
    ...
    try {
      ...
      const transactions = await Promise.all(
        response.transactions.map((blockTransaction) => {
          return new Transaction(blockTransaction.transaction, blockTransaction.block_identifier.index);
        })
      );
